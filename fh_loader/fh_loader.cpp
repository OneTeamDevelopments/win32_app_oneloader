/*
** OneLoader
** Coded By Caner Aydýn For OneLabs
*/
...
char* find_file(char *filename, char ShowToScreen, bool readBack=false);
...
int main(int argc, char *argv[])
{
	...
	for (i = 1; i < (SIZE_T)argc; i++) // skip 0 since argv[0] is fh_loader.exe
	{
		...
		if (strncmp(MyOpt, "mainoutputdir=", MAX(strlen(MyOpt), strlen("mainoutputdir="))) == 0)
		{
	...
	*/
	...
	PromptUser = 0; // this turns on here, user can turn off below with --noprompt
	...
	for (i = 1; i < (SIZE_T)argc; i++) // skip 0 since argv[0] is fh_loader.exe
	{
		...
		else if (strncmp(MyOpt, "rawxml=", MAX(strlen(MyOpt), strlen("rawxml="))) == 0)
		{

			std::string temp = base64_decode(MyArg);
			char * b64 = new char[temp.length() + 1];
			strcpy(b64, temp.c_str());

			// Save into XMLFileTable
			AppendToBuffer(tx_buffer, b64, FIREHOSE_TX_BUFFER_SIZE);
...
char* find_file(char *filename, char ShowToScreen, bool readBack)
{
	SIZE_T i = 0;
	...
	if(!readBack)dbg(LOG_WARN, "Couldn't find the file '%s', returning NULL", filename);
	return NULL;
}
...
static firehose_error_t handleRead()
{
	...
	FileToSendWithPath = find_file(fh.attrs.filename, 1, true);
